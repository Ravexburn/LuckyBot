			...
			.then((ignored) => {
				...
				return notify.forEachKeyword((keyword, userID) => {
					...
					notifications.forEach((keywordSet, userID) => {
						...
						member.send(embed); */
						member.send(`:round_pushpin: User **${message.author.username.replace(/\*/g, '\\*').replace(/\_/g, '\\_').replace(/\~/g, '\\~').replace(/\`/g, '\\`')}** **(${message.author})** has mentioned ${keywords} in ${message.channel} on \`${guild.name}:\` \`\`\`${msg}\`\`\``);
					});
				}).catch(() => {
					console.error;
				});
